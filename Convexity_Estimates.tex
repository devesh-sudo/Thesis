...
\section{Cones and Symmetric polynomials}
...
\end{proposition}
This establishes a hierarchy of convexity with last one being uniformly convex where the principal curvature vector $ \vec{\kappa} = (\kappa_{1}, \ldots, \kappa_{n}) \in \Gamma_{n} $ for all points in the hypersurface. 
...
\begin{proof}
    ...
    Multiplying this equation by $ g_{+}^{p}H^{-\sigma} $ and integrating by parts to get \begin{align*}
        -p \int \frac{1}{H^{\sigma}}g_{+}^{p-1}|\nabla g|^{2}+ \sigma \int \frac{g_{+}^{p}}{H^{1+\sigma}}\left< \nabla H, \nabla g \right> & = 
    \end{align*}
    \end{proof}
    ...
    \begin{lemma}
        [Stampacchia iteration] Let $ \psi: [k_{0},\infty) \to \R $ be a non-negative, non-increasing function which satisfies \begin{equation}
    ...
    We complete the proof of \cref{mainthm} using Stampacchia iteration which gives an $ L^{\infty} $ bound from the $ L^{p} $ bounds.
    \vspace{1pt}
    \begin{proof}
