...
\begin{proof}
    ...
    Now calculating the Laplacian, \begin{align*}
...
for all time $ t \in [0,T) $. This also implies $ g_{\sigma, \eta} \le c_{0}H^{\sigma} $ which is an upper bound but as $ H $ blows-up this isn't sufficient to prove the uniform bound.
...
\begin{proof}
    ...
    Now similar to time derivative we calculate inner product of $ \nabla g $ with $ \nabla H $, \begin{align*}
    ...
    which gives \begin{align}
        ...
        & + p \int \left( (2-\sigma) \frac{g_{+}^{p}}{H^{1+\sigma}}+ 2(1+\eta) \frac{g_{+}^{p-1}}{H} \right)\left< \nabla g, \nabla H \right>d \mu \nonumber \\
        & -2 \int \left( \frac{g_{+}^{p+1}}{H^{2+\sigma}}+ (2+\eta) \frac{g_{+}^{p}}{H^{2}} \right) | \nabla H|^{2}d \mu \label{ggg}
    \end{align}

    From \cref{tensored} $ -2Z \ge \eta H^{2}|A|^{2}  $ and using utilizing $ g \le c_{0}H^{\sigma} $  in \cref{ggg}, we get for any $ \beta > 0 $ 

    \begin{align}
        \eta \int g_{+}^{p}|A|^{2}d \mu & \le c_{0} 
    \end{align}
    \end{proof}
    ...
    \end{proposition}
    \begin{proof}
        Choose $ \beta  \sim p^{-\frac{1}{2}}$ and $ \sigma \sim c p^{-\frac{1}{2}} $
    \end{proof}
