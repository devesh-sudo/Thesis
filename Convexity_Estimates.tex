...
\begin{proof}
    ...
    We complete the proof of theorem \cref{1} using Stampacchia iteration which gives an $ L^{\infty} $ bound from the $ L^{p} $ bounds.
    \vspace{1pt}
    \begin{proof}
        [Stampacchia iteration]

         Let $ k \ge k_{0} $, where $$ k_{0} = \sup_{\sigma \in [0,1]} \sup_{\mathcal{M}_{0}} g_{\sigma, \eta} $$
        Define $ v = (g_{\sigma,\eta} - k)_{+}^{\frac{p}{2}} $ and $ A(k,t) = \{x \in \mathcal{M}_{t} : v(x,t)>0\} $. Differentiating $ v $ with respect to time we get for $ p $ large enough (similar to Lemma 2.1.6 \cref{2})
        \begin{equation}
        ...
        Let $ t_{0} \in [0,T] $ be the time when $ \sup_{t \in [0,T)} \int_{\mathcal{M}_{t}} v^{2} d \mu $ is attained (we let $ t_{0}=T $ if it is not attained in the interior). Integrating \cref{dv2dt} from $ 0 $ to $ t_{0} $, \begin{equation}
            \int_{\mathcal{M}_{t_{0}}} v^{2}d \mu+ \frac{1}{2C(n)} \int_{0}^{t_{0}} \left( \int_{\mathcal{M}_{t}}v^{2q} d \mu\right)^{\frac{1}{q}}dt \le c_{0} \sigma p \int_{0}^{t_{0}} \int_{A(k,t)}H^{2}g_{\sigma,\eta}^{p}d \mu dt
        \end{equation}
        Where we used the fact that $ k>k_{0} \ge \sup_{\mathcal{M}_{0}}g_{\sigma,\eta} $ so $ \int_{\mathcal{M}_{0}}v^{2}d \mu = 0 $. Now integrating \cref{dv2dt} from $ t_{0} $ to $ T $, \begin{equation}
            \int_{\mathcal{M}_{T}}v^{2}d \mu - \int_{\mathcal{M}_{t_{0}}} v^{2}d \mu + \frac{1}{2C(n)} \int_{t_{0}}^{T} \left( \int_{\mathcal{M}_{t}}v^{2q} \right)^{\frac{1}{q}} dt \le c_{0}\sigma p \int_{t_{0}}^{T} \int_{A(k,t)} H^{2}g_{\sigma,\eta}^{p} d \mu dt
        \end{equation}
