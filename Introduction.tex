...
\begin{proof}
    ...
    \[ \sup_{\mathcal{M} \times [0,T)} |\nabla^{m}A|^{2} \le C_{m}\]
    for some constant $ C_{m} < \infty $ as $ |\nabla^{m-1}A| $ is bounded.
...
\begin{proof}[ of \cref{longtimeexistence}]
    Assume on the contrary that $ \sup_{M^{n} \times [0, T)}|A|^{2} \le C $. Our aim is to show that the manifold $X(\cdot, t)= \mathcal{M}_{t} $ converges to a smooth limit $ \mathcal{M}_{T} $ as $ t \to T $ which will give a contradiction by applying short time existence of mean curvature flow at $ t  = T $. 
\end{proof}
