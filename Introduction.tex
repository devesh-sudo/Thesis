...
\[ \Gamma_{ij}^{k} \partial_{k}X = (\partial_{i}( \partial_{j}X))^{T}.\]
...
where $ V $ is some extension of $ v $. As $ A $ is symmetric, the Weingarten map is a self-adjoint operator with real eigenvalues. The ordered eigenvalues of $ L_{p} $ 
\[ \kappa_{1}(p) \le \dots \le \kappa_{n}(p) \]
are called \textbf{principal curvatures}. If we switch the sign of the normal, the principal curvatures flip the sign as well since the Weingarten map flips the sign. The trace of the Weingarten map is called the \textbf{mean curvature} and is denoted by $ H $. So in terms of principal curvatures, 
...
\end{proof}
We can change the definition to include only the normal part as well. \begin{defn}
...
\[ \frac{\partial \psi_{t}}{\partial t}(p) = dX_{t}^{-1}\left( ( \frac{\partial X}{\partial t}(p,t))^{\perp} - \frac{\partial X}{\partial t}(p,t) \right), \qquad \psi_{0} = \operatorname{id}_{M} \]
